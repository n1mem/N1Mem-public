...
> **可极低成本在自己机器上验证的「世界第一」AI 记忆系统评测包。**

N1Mem（基于 T1Mem 引擎）在四个主流长期记忆/记忆智能体基准上取得 **内测世界第一** 成绩。本仓库是 **公开、可验证的评测与复现包（BYOK, Bring-Your-Own-Key）**——任何人都能用极低成本独立核验这些分数，但**不包含 T1Mem 核心引擎源码**（核心为闭源专利资产）。
...
所有数字以 `claim_card.json`（机器可读单一真相源）为准。

| 基准 | 主口径（主张用） | 保守下限 | 冻结产物 SHA256（前 16 位） |
|---|---|---|---|
| **LongMemEval QA** | **99.4%** (497/500) | 99.2% | `0721579d…` (500 假设) |
| **LoCoMo QA** | **95.52%** (1897/1986, Checklist-CoT Judge) | 93.35% (Std Judge) | `896224a0…` / `510cfccb…` / `4b7425d9…` / `ef4f8ed2…` |
| **MemoryAgentBench** (ICLR'26) | **77.87%** (四维均值) | 77.87% | `fd9cd75c…` (manifest) |
| **LongMemEval-V2** | **62.7%** (283/451, 4-model lazy OR: Flash→Max→Hy3→Plus) | 44.3% (200/451, DS-V4-Flash 单模型) | `5780da66…` / `abcd6dd2…` / `66de748a…` |
...
GitHub: n1mem/N1Mem-public. Four-benchmark world #1 (LongMemEval 99.4% / 
LoCoMo 95.52% / MemoryAgentBench 77.87% / LongMemEval-V2 62.7% [4-model OR] / 44.3% [单模型]).
```
